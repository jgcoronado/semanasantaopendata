// Genera src/data/velocidades-2026.json a partir de:
//   - public/geojson/2026/*.geojson   (trazado real de cada hermandad: LineString + Points de calle)
//   - src/data/horarios-2026.json     (tiempos en salida, campana, catedral, entrada, …)
...
const horarios = JSON.parse(readFileSync(join(RAIZ, 'src/data/horarios-2026.json'), 'utf8'));
...
const salida = {
  ...
  fuente: 'GeoJSON de recorridos 2026 + horarios-2026.json',
  registros,
};

writeFileSync(join(RAIZ, 'src/data/velocidades-2026.json'), JSON.stringify(salida, null, 2) + '\n');

// ── informe en consola ───────────────────────────────────────────────────────
const f0 = (n) => (n == null ? '   —' : String(n).padStart(4));
console.log(`\n✔  ${registros.length} hermandades → src/data/velocidades-2026.json (velocidades en m/h)\n`);
